    ...
    <div className="relative h-full w-full">
        ...
        <div className='absolute text-white right-[20px] top-[20px] cursor-pointer' onClick={() => onClose()}>
            ...
            <div className='h-full w-[400px] z-20 text-center flex flex-col items-center'>
                <span className='text-white font-Nunito text-2xl font-bold'>Granny's<span className='text-[#166534] text-2xl'>Recipes</span></span>
                <p className='text-[#1FB137] mt-5 text-lg font-bold'>Sign in with your email address and password</p>
                <form onSubmit={handleSubmit} className='text-left w-[340px] mt-10'>
                    <label className='text-[#1FB137] text-xl font-bold'>
                        Email address:
                        <br />
                        <input type="email" name="email" id="email" required className='border-[#1FB137] bg-black border w-full text-lg p-2' onChange={(e) => setEmail(e.target.value)}/>
                    </label>
                    <label className='text-[#1FB137] mt-5 text-xl font-bold block'>
                        Password:
                        <br />
                        <input type="password" name="password" id="password" required className='border-[#1FB137] text-lg bg-black border w-full p-2' onChange={(e) => setPassword(e.target.value)}/>
                    </label>
                    <button className='text-[#1FB137] underline mt-3 text-base font-bold block ml-auto flex gap-6 inline-block text-base' onClick={handleForgotPassword} >
                        Forgot password?  
                    </button>
                    ...
                    <div className="flex justify-center mt-12">
                        <button type="submit" value="Sign in" className='bg-[#166534] w-[130px] h-[45px]  rounded-3xl text-white text-2xl self-center' >
                            Sign in
                        </button>
                    </div>
                </form>
