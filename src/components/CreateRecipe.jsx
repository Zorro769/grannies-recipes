...
const closeDialog = () => {
  ...
  const handleSubmit = async (e) =>  { 
    ...
    .filter(ingredient => ingredient && ingredient.original && ingredient.original.trim() !== '')
    .map(ingredient => ({ original: ingredient.original }));

      
    formData.append('extendedIngredients',JSON.stringify(serializedIngredients));
  ...
  const handleIngredientChange = (index, value) => {
    ...
    const filteredIngredients = newIngredients.filter(
      (ingredient) => ingredient !== null && ingredient !== undefined && ingredient.original !== undefined
    );
            ...
            <div className='h-full w-[400px] z-20 text-center flex flex-col items-center'>
                ...
                <form onSubmit={handleSubmit} className='text-left w-[340px] mt-10'>
                    <label className='text-[#1FB137] text-base font-bold '>
                        Name of recipe:
                        <br />
                        <input type="name" name="name" id="name" placeholder='Enter title of recipe' className=' border-[#1FB137] bg-black border w-full py-2 pl-4 pr-10' onChange={(e) => setName(e.target.value)}/>
                    </label>
                    ...
                    <label className='text-[#1FB137] text-base font-bold inline-block mt-5 w-full'>
                        Dish type:
                        <select id="dishSelect" className="border-[#1FB137] bg-black border w-full py-2 pl-4 pr-10" onChange={(e) => setDishType(e.target.value)} value={dishType}>
                    ...
                    <label className='text-[#1FB137] text-base font-bold inline-block mt-5 w-full'>
                        Total time(in min):
                        <br />
                        <input type="number" placeholder='Enter total time' onWheel={(e) => e.target.blur()} name="time" id="time" className=' border-[#1FB137] bg-black border w-full py-2 pl-4 pr-10' onChange={(e) => setTotalMin(e.target.value)}/>
                    </label>
                    ...
                    <span className='text-[#1FB137] text-base font-bold inline-block mt-5 w-full radio-container'>Ingredients</span>
                        ...
                        <br />
                            <input type="text" placeholder='Enter ingredient' value={ingredient.original} className='text-[#1FB137] text-base border-[#1FB137] bg-black border w-full py-2 pl-4 pr-10' onChange={(e) => handleIngredientChange(index, e.target.value)}/>
                        </label>
                        <br />
                        </div> ))}
                    <span className='text-[#1FB137] text-base font-bold inline-block mt-5 w-full radio-container'>Instructions</span>
                    <br />
                    <textarea placeholder='Enter instructions' name="dishType" onChange={(e) => setInstructions(e.target.value)} className='border-[#1FB137] text-[#1FB137] bg-black border w-full py-2 pl-4 pr-10 resize-none' style={{ height: '200px' }} />
                    <label className='text-[#1FB137] text-base font-bold inline-block mt-2 w-full radio-container'>
                        Pick an image
                    </label>
                    <br />
                    <input type="file" name="image" onChange={handleFileChange} className='hidden' id="fileInput" accept='image/*'/>
                    <label htmlFor="fileInput" className={`bg-[#166534] w-[130px] h-[45px] rounded-3xl text-white text-xl text-center cursor-pointer inline-block mt-2 px-4 py-2 ${selectedFile ? 'bg-gray-700' : ''}`}>
                        {'Choose'}      
                    </label>
                    <br/><br />
                    {selectedFile ? (
